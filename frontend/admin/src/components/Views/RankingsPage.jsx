...
const STUDENTS = [
  { id: "MPEC23CS031", name: "Priya Nair", branch: "CSE (AI)", cgpa: 9.5, aptitude: 91, communication: 88, projects: 95, leetcode: 601 },
  { id: "MPEC23CS014", name: "Ishita Verma", branch: "CSE (AI)", cgpa: 9.2, aptitude: 89, communication: 92, projects: 90, leetcode: 530 },
  { id: "MPEC23CS001", name: "Aarav Sharma", branch: "CSE", cgpa: 8.9, aptitude: 85, communication: 80, projects: 88, leetcode: 412 },
  { id: "MPEC23CS018", name: "Ananya Singh", branch: "CSE", cgpa: 8.6, aptitude: 82, communication: 84, projects: 79, leetcode: 349 },
  { id: "MPEC23CS022", name: "Sneha Pillai", branch: "CSE", cgpa: 8.1, aptitude: 78, communication: 81, projects: 74, leetcode: 287 },
  { id: "MPEC23ME004", name: "Devansh Joshi", branch: "ME", cgpa: 7.8, aptitude: 70, communication: 75, projects: 68, leetcode: 95 },
  { id: "MPEC23IT007", name: "Rohan Gupta", branch: "IT", cgpa: 7.4, aptitude: 64, communication: 70, projects: 60, leetcode: 120 },
  { id: "MPEC23EC009", name: "Karan Mehta", branch: "ECE", cgpa: 6.9, aptitude: 58, communication: 66, projects: 52, leetcode: 64 },
];

// Composite weighting — mirrors the CampusIQ readiness scoring logic
const WEIGHTS = { cgpa: 0.25, aptitude: 0.25, communication: 0.2, projects: 0.3 };

function compositeScore(s) {
  const cgpaPct = (s.cgpa / 10) * 100;
  return Math.round(
    cgpaPct * WEIGHTS.cgpa +
      s.aptitude * WEIGHTS.aptitude +
      s.communication * WEIGHTS.communication +
      s.projects * WEIGHTS.projects
  );
}
...
function RankBadge({ rank }) {
  if (rank <= 3) {
    return (
      <div
        className={`flex items-center justify-center size-8 rounded-full font-medium text-sm ${RANK_STYLES[rank]}`}
      >
...
export function RankingsPage() {
  ...
  const ranked = useMemo(() => {
    const withScores = STUDENTS.map((s) => ({ ...s, score: compositeScore(s) }));
    return withScores
          ...
          {infoOpen && (
            <div className="absolute right-0 mt-2 w-64 rounded-lg border border-border bg-card shadow-lg p-4 z-10 text-sm">
              ...
              <ul className="space-y-1.5 text-muted-foreground">
                <li className="flex justify-between"><span>CGPA</span><span>25%</span></li>
                <li className="flex justify-between"><span>Aptitude</span><span>25%</span></li>
                <li className="flex justify-between"><span>Communication</span><span>20%</span></li>
                <li className="flex justify-between"><span>Projects</span><span>30%</span></li>
              </ul>
            </div>
          )}
        </div>
